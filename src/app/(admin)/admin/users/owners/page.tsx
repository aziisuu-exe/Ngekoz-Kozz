...
import { UserActions } from "../customers/_components/user-actions";
import { IconChevronLeft, IconChevronRight, IconSearch, IconUsers, IconUser, IconCheck, IconX, IconPhone, IconMail, IconCalendar, IconEye, IconBriefcase, IconGenderMale, IconBuilding } from "@tabler/icons-react";
...
export default async function AdminKelolaPemilikPage({ searchParams }: PageProps) {
  ...
  return (
    <div className="p-6 md:p-8 w-full max-w-7xl mx-auto space-y-6 relative">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-5 border-b border-gray-100">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 tracking-tight">Kelola Pemilik Kos</h1>
          <p className="text-sm text-gray-500 mt-0.5">Total mitra pemilik terdaftar: {totalItems} pengguna</p>
        </div>
                ...
                <tr>
                  <td colSpan={7} className="py-16 text-center text-gray-400 font-medium">
                    <div className="flex flex-col items-center justify-center gap-2">
                      ...
                      <span>Tidak ada data pemilik kos yang ditemukan</span>
                    </div>
                  </td>
                </tr>
              ) : (
                data.map((user: any) => (
                  <tr key={user.id} className="hover:bg-gray-50/40 transition-colors">
                    ...
                    <td className="py-4 px-6 text-right">
                      <Link
                        href={`/admin/users/owners?show=${user.id}&${cleanParamsString}`}
        ...
        {totalPages > 1 && (
          <div className="px-6 py-4 bg-gray-50/50 border-t border-gray-50 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <Link
                href={`/admin/users/owners?page=${currentPage - 1}${currentSearch ? `&search=${currentSearch}` : ""}`}
        ...
        <div className="fixed inset-0 z-50 bg-gray-900/40 backdrop-blur-xs flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white w-full max-w-md rounded-2xl border border-gray-100 shadow-xl overflow-hidden p-6 space-y-5 animate-in zoom-in-95 duration-150 max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex items-center justify-between border-b border-gray-50 pb-3">
              <h3 className="text-base font-bold text-gray-900">Profil Lengkap Pemilik Properti</h3>
              <Link 
                href={`/admin/users/owners?${cleanParamsString}`}
            ...
            <div className="flex flex-col items-center text-center space-y-2">
              ...
              <div>
                ...
                <span className="text-xs text-emerald-600 font-semibold uppercase tracking-wider bg-emerald-50 px-2.5 py-0.5 rounded-md border border-emerald-100 block w-max mx-auto mt-1.5">
                  Partner Owner
                </span>
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider block">Deskripsi / Bio</span>
            ...
            <div className="bg-gray-50/60 rounded-xl border border-gray-100 p-4 space-y-3 text-xs">
              ...
              <div className="flex items-center gap-3 text-gray-600">
                ...
                <span>Bisnis Utama: <strong className="text-gray-900 font-semibold">{selectedUser.pekerjaan}</strong></span>
              </div>
              <div className="flex items-center gap-3 text-gray-600">
                <IconCalendar size={16} className="text-gray-400 shrink-0" />
                <span>Kemitraan Sejak: {new Date(selectedUser.created_at).toLocaleDateString("id-ID", { day: 'numeric', month: 'long', year: 'numeric' })}</span>
              </div>
            </div>

            <div className="grid grid-cols-2 gap-3">
              <div className="bg-white border border-gray-100 p-3 rounded-xl text-center shadow-2xs">
                <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider block">Total Properti</span>
                <span className="text-xl font-black text-gray-900 block mt-0.5 flex items-center justify-center gap-1">
                  <IconBuilding size={16} className="text-gray-400" />
                  {selectedUser.count} Kos
                </span>
              </div>
              <div className="bg-white border border-gray-100 p-3 rounded-xl text-center shadow-2xs">
                <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider block">Validasi Hukum</span>
